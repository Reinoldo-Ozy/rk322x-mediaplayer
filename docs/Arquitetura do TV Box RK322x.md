...
- **Receptor de rede** — tudo aqui foi validado com arquivo local. O transporte (Sunshine/RTP), o áudio e o autostart ainda não foram montados.
- **Áudio HDMI** — nunca testado. O log mostra `HDMI: Unknown ELD version 0`, então há trabalho ali.
- **IP fixo** — a box está em `192.168.10.159` por DHCP (perdeu o `.158` para um roteador TP-Link no meio da investigação).
- **Publicar a correção** — o README do projeto e o post no fórum Armbian ainda afirmam "720p", o que está desatualizado por baixo.
